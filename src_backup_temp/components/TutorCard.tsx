...
export const TutorCard: React.FC<TutorCardProps> = React.memo(({ 
  ...
  const feeRaw = getValue(['Fee/Month', 'feeMonth', 'Fee', 'fee'], 'Flexible').toString();
  const fee = (() => {
    const numMatch = feeRaw.match(/(\d+)/);
    if (!numMatch) return feeRaw;
    const num = numMatch[1];
    let unit = '';
    if (feeRaw.toLowerCase().includes('hour') || feeRaw.toLowerCase().includes('hr')) unit = '/hr';
    else if (feeRaw.toLowerCase().includes('month') || feeRaw.toLowerCase().includes('mo')) unit = '/mo';
    else if (feeRaw.toLowerCase().includes('class')) unit = '/class';
    return `₹${num}${unit}`;
  })();
